...
class ActionGraph(object):
    ...
    def get_is_dest_func_goal(self):
        blizz = self.blizzz[0]
        idim, jdim = blizz.shape
        return lambda u: u[0] == idim - 1 and u[1] == jdim - 2

    def get_is_dest_func_start(self):
        blizz = self.blizzz[0]
        return lambda u: u[0] == 0 and u[1] == 1

    def heuristic_start_to_goal(self):
        blizz = self.blizzz[0]
        idim, jdim = blizz.shape
        return lambda u: abs(idim - 1 - u[0]) + abs(jdim - 2 - u[1])

    def heuristic_goal_to_start(self):
        blizz = self.blizzz[0]
        idim, jdim = blizz.shape
        return lambda u: abs(0 - u[0]) + abs(1 - u[1])
...
def main(filename, method="astar"):
    grid = read(filename)
    G = ActionGraph(grid)
    u = (0, 1, 0)
    for a in G[u]:
    ...
    start = (0, 1)
    goal = (grid.shape[0]-1, grid.shape[1]-2)

    is_dest_goal = G.get_is_dest_func_goal()
    is_dest_start = G.get_is_dest_func_start()
    if method == "dijkstra":
        path, cost_1 = dijkstra(G, (start[0], start[1], 0), is_dest_goal, debug_freq=0)
    else:
        Hfunc = G.heuristic_start_to_goal()
        path, cost_1 = a_star(G, (start[0], start[1], 0), is_dest_goal, Hfunc, debug_freq=0)
    print(path)
    print("cost = ", cost_1)

    initial = (goal[0], goal[1], cost_1)
    if method == "dijkstra":
        path, cost_2 = dijkstra(G, initial, is_dest_start, debug_freq=0)
    else:
        Hfunc = G.heuristic_goal_to_start()
        path, cost_2 = a_star(G, initial, is_dest_start, Hfunc, debug_freq=0)
    print(path)
    print("cost = ", cost_2)

    initial = (start[0], start[1], cost_1 + cost_2)
    if method == "dijkstra":
        path, cost_3 = dijkstra(G, initial, is_dest_goal, debug_freq=0)
    else:
        Hfunc = G.heuristic_start_to_goal()
        path, cost_3 = a_star(G, initial, is_dest_goal, Hfunc, debug_freq=0)
    print(path)
    print("cost = ", cost_3)

    print("total cost =", cost_1 + cost_2 + cost_3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", default="astar", help="dijkstra|astar")
